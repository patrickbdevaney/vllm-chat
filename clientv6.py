...
<head>
    ...
    <script>
        ...
        async function sendPrompt() {
            ...
            let latencyStartTime = Date.now();  // Start time for latency measure
            let firstChunkReceived = false;
            let measuredLatency = null; // Store latency once measured
            ...
            try {
                ...
                while (true) {
                    ...
                    // Update tokens and elapsed time
                    const tokens = chunk.split(/\s+/).filter(Boolean).length;
                    totalTokens += tokens;
                    const elapsedTime = (Date.now() - startTime) / 1000;
                    const tokensPerSecond = (totalTokens / elapsedTime).toFixed(2);
                    
                    // Measure latency when the first non-empty chunk is received
                    if (!firstChunkReceived && chunk.trim().length > 0) {
                        measuredLatency = (Date.now() - latencyStartTime) / 1000;
                        firstChunkReceived = true;
                    }
                    
                    // Update stats with measured latency if available
                    statsElement.textContent = 
                        `Tokens: ${totalTokens} | Time: ${elapsedTime.toFixed(2)}s | Tokens/sec: ${tokensPerSecond}` +
                        (measuredLatency !== null ? ` | Latency: ${measuredLatency.toFixed(2)}s (Click to First Response)` : "");
                }
            } catch (err) {
